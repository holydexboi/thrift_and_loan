...
router.post("/deposit", auth, async (req, res) => {
  ...
  if (!req.body.transactionCode)
    return res.status(400).send("Please enter transaction code or tell number");
  if (!req.body.paymentType)
    return res.status(400).send("Please enter payment type");
  console.log(req.body.paymentType);
  Saving.getSavingsBalance(1)
    .then((saving) => {
      Transaction.create({
        id: transactionId,
        amount: parseFloat(req.body.amount),
        savings_id: 1,
        balance: saving + parseFloat(req.body.amount),
        transaction_code: parseInt(req.body.transactionCode),
...
router.post("/withdraw", auth, async (req, res) => {
  ...
  if (!req.body.amount) return res.status(400).send("Please enter amount");
  if (!req.body.beneficiaryAccount)
    return res
      .status(400)
      .send("Please enter transaction code or teller number");
  if (!req.body.bank) return res.status(400).send("Please enter bank");
  Saving.getSavingsBalance(1)
    .then((saving) => {
      Transaction.create({
        id: transactionId,
        ...
        savings_id: 1,
        balance: saving - parseFloat(req.body.amount),
        transaction_code: req.body.beneficiaryAccount,
